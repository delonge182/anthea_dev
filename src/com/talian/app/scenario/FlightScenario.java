...
public class FlightScenario implements MboConstants {
	...
	public HashMap<String, Port> availablePorts ;
	public HashMap<String, Port> refuelingPorts ;
	...
	public void setMbo (MboRemote mbo) {
		this.mbo = mbo ;
	}
...
//					Double d = vp.getFueluploadneed() + usedFleet.eew + (la == null ? 0.0 : la.getWeightonboard()) + cfg.getDouble("crewweight");
					
//					mbo.setValue("TOW", d.intValue());
//					mbo.setValue("MAXFUELUPLOAD", vp.get);
					mbo.setValue("FUELUPLOAD", vp.getFueluploadneed());
//					mbo.setValue("MAXPAYLOAD", vp.getPayload());
					mbo.setValue("PAYLOAD", vp.getPayload());
					mbo.setValue("FUELVALIDITY", vp.isFuelvalid());
					mbo.setValue("ISVALID", usedRoute.isValid());
					mbo.setValue("POBVALIDITY", vp.isPobvalid());
					mbo.setValue("WOBVALIDITY", vp.isWOBValid());
					mbo.setValue("PAXONOFF", vp.getPaxon() +"/"+ vp.getWeightoff());
//					mbo.setValue("WEIGHTONOFF", );
					mbo.setValue("PAXON", vp.getPaxon());
					mbo.setValue("PAXOFF", vp.getPaxoff());
					mbo.setValue("FUELNEED", vp.getFuelneed());
...
//			mbo.getThisMboSet().save();
			...
			while (it4.hasNext()) {
				...
				boolean ispobvalid = false;
				boolean iswobvalid = false;
				String paxonoff;
				int paxon = 0;
				int paxoff = 0;
				Double fuelneed = 0.0;
				...
				while (vp != null) {
					...
					fueluploadneed = vp.getFueluploadneed();
					payload = vp.getPayload();
					isfuelvalid = vp.isFuelvalid();
					ispobvalid = vp.isPobvalid();
					iswobvalid = vp.isWOBValid();
					paxonoff = vp.getPaxon() +"/"+ vp.getPaxoff();
					paxon = vp.getPaxon();
					paxoff = vp.getPaxoff();
					fuelneed = vp.getFuelneed();
					...
					Double d =  fueluploadneed + usedFleet.eew + wob1 + this.getConfig().defaultCrewWeight;
					mbo.setValue("TOW", d.intValue());
//					mbo.setValue("MAXFUELUPLOAD", vp.get);
					mbo.setValue("FUELUPLOAD", fueluploadneed);
//					mbo.setValue("MAXPAYLOAD", vp.getPayload());
					mbo.setValue("PAYLOAD", payload);
					mbo.setValue("FUELVALIDITY", isfuelvalid);
					mbo.setValue("ISVALID", usedRoute.isValid());
					mbo.setValue("POBVALIDITY", ispobvalid);
					mbo.setValue("WOBVALIDITY", iswobvalid);
					mbo.setValue("PAXONOFF", paxonoff);
//					mbo.setValue("WEIGHTONOFF", );
					mbo.setValue("PAXON", paxon);
					mbo.setValue("PAXOFF", paxoff);
					mbo.setValue("FUELNEED", fuelneed);
				}
//				scenassset.save();
			}
	...
	static public FlightScenario readfromMBO (MboRemote mbo) throws RemoteException, MXException {
		...
		if (!mbo.getString("flightsession").equals("") &&  mbo.getString("reservedate")!= null) {
			MboSetRemote distances = mbo.getMboSet ("distance") ;
			MboRemote dist = distances.moveFirst() ;
			while (dist != null) {
				Distance d0 = Distance.readfromMBO(fs,dist) ;
				System.err.println("scenarioid: "+ mbo.getString("scenarioid"));
